...
2026-09-04 重新生成的候选组件包在 loader 完成 DRAM 初始化后未重新枚举为
FES，于 8% 终止；没有 NAND committed bytes，校验未运行，未自动重试。该次
结果记录在 `logs/fes-validation-20260904.jsonl`，不能称为烧录成功，也不改变
旧 v5 精确哈希集合的已验证状态。
